...
def create_app(state_manager: StateManager, execution_controller: ExecutionController) -> FastAPI:
    ...
    def resolve_incident(req: ResolveRequest, token: str = Depends(verify_token)):
        ...
        if action == "ALLOW":
            ...
            state_manager.clear_incident(incident_id)
            state_manager.add_log(LogType.INFO, f"Incident {incident_id} resolved: ALLOW. Execution resumed normally.")
        elif action == "BLOCK" or action == "BLOCK_COMMAND":
            execution_controller.resume(incident_id, action)
            state_manager.clear_incident(incident_id)
            state_manager.add_log(LogType.INFO, f"Incident {incident_id} resolved: BLOCK_COMMAND. Malicious payload discarded before resuming.")
        elif action == "QUARANTINE":
            execution_controller.terminate(incident_id, action)
            state_manager.clear_incident(incident_id)
            state_manager.add_log(LogType.WARNING, f"Incident {incident_id} resolved: QUARANTINE. Agent killed.")
        else:
